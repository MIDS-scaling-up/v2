...
```
aws ec2 create-key-pair --key-name MIDSkeypair  --query 'KeyMaterial'  --output text > MIDSkeypair.pem
...
```
aws ec2 run-instances --image-id ami-0bcc094591f354be2 --security-group-ids sg-0d9ac24e91a90bb9e  --instance-type t2.large --key-name MIDSkeypair (notice that depending on your region some image AMI id-s are different and might not work)
